...
#'   count `K` must satisfy `1 <= K <= N - 1`. An explode marker may follow the
#'   die size, before any keep or drop selector or modifier: `!` rerolls a
#'   maximum-face die once and sums both faces, `!!` rerolls while the maximum
...
#'   required and each keep or drop selector applies within its own term only.
